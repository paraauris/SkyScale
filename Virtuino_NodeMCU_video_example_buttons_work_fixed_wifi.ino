...
void setup() {
  ...
  //Address gateway(192, 168, 2, 1);         // set gateway to match your network
 ...
 //---- Enter your setup code below
  ...
  pinMode(D5,INPUT);                 // connect a relay or a led to this pin.  On Virtuino panel add a switch to pin D3
  pinMode(D6,OUTPUT);                 //  connect a switch.  On Virtuino panel add a Led to pin D5
...
void loop() {
  virtuino.run();
   ...
   if (v!=storedValue) {                          
    ...
    if (v==1) digitalWrite(D0,1);
    else digitalWrite(D0,0);
    
    Serial.println("-------Virtual pin DV1 is changed to="+String(v));
    if (v==1) digitalWrite(D1,1);
    else digitalWrite(D1,0);
    
   Serial.println("-------Virtual pin DV2 is changed to="+String(v));
    if (v==1) digitalWrite(D2,1);
    else digitalWrite(D2,0);

    Serial.println("-------Virtual pin DV3 is changed to="+String(v));
    if (v==1) digitalWrite(D3,1);
    else digitalWrite(D3,0);
    storedValue=v;
   ...
   // storedValue=v;
    ...
    if (t>storedTime+300){
      ...
      if (counter>9999) counter=0;    // limit = 9999
      storedTime = t; 
