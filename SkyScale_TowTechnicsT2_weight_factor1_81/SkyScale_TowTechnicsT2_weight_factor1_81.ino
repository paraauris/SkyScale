...
//   Serial.println(WiFi.localIP());
  ...
  //----   2. Settings as Access point - Create a private Wifi Network. Enable the next five lines to use module as Acces point 
   ...
   boolean result = WiFi.softAP("TowTechnicsT2", "123459876");      // SSID: NodeMCU   Password:12345678  App security password:1234
...
//  lcd.print (" m");
  ...
  virtuino.vMemoryWrite(SPEED_VIRTUAL,(long)wireSpeed);
  virtuino.vMemoryWrite(DISTANCE_VIRTUAL,(long)wireLength);
...
//  while (!scale.isReady());
  ...
  printTime();
  double traction = scaleFactor * weight * scale.get_units(); // daugiklis prinktas be virsutinio kasetės ritinuko, virvė eina tiesiai per vamzdžio centrą.
